"""Module 06 — Trust & Fraud Prevention (doc/SRS/06, doc/multi-agent-architecture/06).

No route prefix, matching `recruitment.py`/`hackathons.py`'s flat endpoint style (doc 06
§6 has no shared prefix either). This is the highest ethical-risk-surface module in the
platform (doc 06 §1) — every endpoint here is written to the letter of §7/§8's binding
constraints:

- A `raised` flag is NEVER wired to any automatic side-effect (no Talent Score change, no
  visibility/ranking change, no Copilot filter). This router doesn't touch Module 01's
  `talent_scores`/Module 02's `match_scores` at all — only its own tables.
- `PATCH /flags/{id}/review` rejects (422) a transition to `upheld` with empty
  `review_notes` — enforced here, not just as a DB constraint or frontend nicety.
- `fraud_flags.evidence` is always a real, non-empty evidence payload — every flag this
  router creates carries the actual signal evidence plus a Fraud Risk Report Agent
  narrative (or a deterministic evidence-only fallback if Sonnet is unavailable), never a
  bare accusation.
- Photo-hash / resume-fingerprint checks call `_require_consent` (imported from
  `candidates.py`, not duplicated) for `perceptual_photo_hash` / `resume_parsing` before
  running, per doc 06 §8.

Cross-module reads only (Module 01's `certifications`/`candidate_profiles`, Module 03's
`submissions`) — this router never writes to another module's tables.
"""
...
async def _persist_check_result(
    ...
    *,
    ...
    """Shared post-processing for every `POST /verification/.../check` endpoint —
    persists a `verification_records` row per signal (the full evidence trail,
    regardless of whether a flag was raised), an `agent_runs` explainability row
    (constraints.md §4 — no silent score/verdict), and conditionally a `fraud_flags` row
    when `verdict['should_flag']` is True. A NEW flag is only created if one doesn't
    already exist in a non-terminal state for this exact subject+flag_type, so re-running
    a check doesn't spam duplicate raised flags for the same underlying issue."""
...
# --- FR-1: Fake Certificate Detection ---
...
# --- FR-2/FR-5: Fake/Plagiarized Project & Submission Detection (shared graph) ---
...
# --- FR-4: Duplicate Profile Detection + FR-3: AI-Generated Content Signal ---
...
async def check_profile_duplicate(
    ...
    """Runs BOTH the Duplicate Profile Detection subgraph (FR-4) and the AI-Generated
    Content Signal subgraph (FR-3) — doc 06 §6 has no separate route for FR-3, and both
    operate on the same profile subject (see `.agents/decisions.md`'s Module 06 entry).
    Photo-hash and text-fingerprint each individually require their own consent
    (`perceptual_photo_hash` / `resume_parsing`) per doc 06 §8 — a missing consent
    degrades that ONE signal to unavailable rather than 403ing the whole check, since the
    other signal(s) may still be legitimately checkable."""
    profile = await _candidate_profile_or_404(db, candidate_id)

    profile_text = _profile_text(profile)

    # `_require_consent` (reused, not duplicated, per the assignment's explicit
    # instruction) raises a 403 when consent is missing — caught here so ONE missing
    # consent degrades only that signal rather than 403ing the whole check, since the
    # other signal may still be legitimately checkable (doc 06 §8 requires consent
    # before EACH of these two specific analyses, not an all-or-nothing gate on the
    # endpoint).
...
# --- FR-6: Candidate Authenticity Score ---
...
# --- FR-8: Candidate-visible flags + dispute flow ---
...
async def get_flag_detail(
    ...
    """Additive endpoint (not in doc 06 §6's literal list) — the evidence viewer and the
    admin flag-audit stub page both need a single-flag detail read with the Dispute
    Review Agent's assistive summary attached; the queue endpoint alone doesn't carry
...
async def review_flag(
    ...
    """The ONLY place a flag can move to `upheld`/`dismissed` — always a human decision
    (doc 06 §7/§8's binding rule). `upheld` REQUIRES non-empty `review_notes`, rejected
    with 422 here at the API layer (not just the DB's nullable-but-unchecked column) per
    the assignment's hard constraint."""
