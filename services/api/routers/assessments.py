"""Module 03 — AI Assessment & Verification System (doc/SRS/03,
doc/multi-agent-architecture/03). No route prefix — mirrors `recruitment.py`'s flat
endpoint style (doc 03 §9's endpoint list has no shared prefix either)."""
...
def _fetch_repo_sample_source(repo_full_name: str, max_size: int = 20_000) -> str:
    """Public-repo-only sample fetch (no candidate GitHub token is ever persisted server
    -side, per FR-1.1's "never a shared/stored token" rule — see state.py's comment in
    Module 01). Grabs the first reasonably-sized Python file it finds at the repo root."""
