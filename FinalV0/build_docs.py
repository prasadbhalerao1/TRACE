...
def build_docx() -> Path:
    ...
    while i < len(lines):
        ...
        if m:
            ...
            # The contents list owns page 1; break before whatever follows it.
            if was_toc and not in_toc:
        ...
        if m:
            ...
            # In the contents list the markers are already correct in the source
            # (1..12 top level, restarting per section). Word's "List Number"
            # style keeps its own counter and flattens the nesting into one long
            # run, so write the marker as literal text instead.
            if in_toc and ordered:
                p = doc.add_paragraph()
                p.paragraph_format.left_indent = Inches(0.28 + 0.26 * (indent // 3))
                p.paragraph_format.space_after = Pt(1)
                r = p.add_run(m.group(2) + " ")
                r.font.size = Pt(11)
                if indent == 0:
                    r.bold = True
                add_runs(p, m.group(3), base_size=Pt(11))
                i += 1
                continue
